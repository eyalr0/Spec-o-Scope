...
enum class AttackType {
	SBOX, TTABLE, MIN_DIST,
};
static AttackSetup setup_attack(const char* victim_binary, attacker_fn_t attacker, int table_cacheline, AttackType attack_type) {
	...
	uintptr_t input1, input2;
	switch(attack_type) {
	case AttackType::SBOX: {
		input1 = page2 + table_cacheline * CACHE_LINE_SIZE + 3*CACHE_LINE_SIZE;
	} break;
	case AttackType::TTABLE: {
		input1 = (uintptr_t) &AES_decrypt;
	} break;
	case AttackType::MIN_DIST: {
		input1 = page1 + PAGE1_CACHELINE * CACHE_LINE_SIZE;
	} break;
	}
	input2 = page2 + table_cacheline * CACHE_LINE_SIZE;
...
static int test_aes_break_graph(int argc, char *argv[]) {
	...
	auto setup_res = setup_attack(victim_binary, fs_ps_attacker, target_line, AttackType::TTABLE);
...
static void test_aes_break_random_key_inner(const char* victim_binary, int start_phase, int test_count, FILE* fout, int target_line, attacker_fn_t attacker) {
	auto setup_res = setup_attack(victim_binary, attacker, target_line, AttackType::TTABLE);
...
Output Format: n traces (one on each line), where each trace is [key,ciphertext,a1,a2].
	a1 and a2 are the attacker's results, or -1 if no event was found.
*/
static int test_aes_break_random_key_ttable(int argc, char *argv[]) {
	const char* victim_binary = "./build/aes_ttable_victim";
	int start_phase = 8;
	int test_count = 4000;
	int target_line = 16;
...

	{
		...
		test_aes_break_random_key_inner(victim_binary, start_phase, test_count, fout, target_line, fs_ps_attacker);
		fclose(fout);
	}
...
static int test_aes_break_random_key_ttable_sr(int argc, char *argv[]) {
	const char* victim_binary = "./build/aes_ttable_victim";
	int start_phase = 8;
	int test_count = 5000;
	int target_line = 16;
	...
	for(int k = 0; k < 1'000; k++) {
		...
		AES_KEY aeskey;
		private_AES_set_decrypt_key(rand_key, 128, &aeskey);

		perform_attack_multiple_ct(setup_res.adata, test_count, [&aeskey](int, uint8_t* input) {
			generate_example_ct_ttable_ge(&aeskey, input, 1, 1, 0);
		}, fout, key_text);
	}
...
static int test_aes_break_graph_sbox(int argc, char *argv[]) {
	...
	auto setup_res = setup_attack(victim_binary, fs_ps_attacker, target_line, AttackType::SBOX);
...
static int test_aes_break_random_key_sbox_sr(int argc, char *argv[]) {
	const char* victim_binary = "./build/aes_sbox_victim";
	int start_phase = 12;
	int test_count = 100;
	int target_line = 17;
	...
	for(int k = 0; k < 5; k++) {
		...
		for(int i = 0; i < test_count; i++) {
			...
			total_tries += perform_attack_multiple_ct_retry(setup_res.adata, 16*64, [&ct](int t, uint8_t* input) {
				memcpy(input, ct, 16);
				int byte_idx = t/64;
				int lsb = t%64;
				input[byte_idx] = (input[byte_idx] & 0xC0) | lsb; 
			}, fout, key_text);
		}
		printf("Total tries: %d, minimum: %d\n", total_tries, test_count*16*64);
	}
...
static int test_min_distinguish(int argc, char *argv[]) {
	const char* victim_binary = "./build/md_min_distinguish_victim";
	int start_phase = 10;
	int test_count = 1000;
	int target_line = 37;
	const char* out_file = "";
	...
	auto setup_res = setup_attack(victim_binary, fs_ps_attacker, target_line, AttackType::MIN_DIST);
...
int test_aes_break(int argc, char *argv[]) {
	...
	// return test_aes_break_random_key_ttable(argc, argv);
	...
	if(strcmp(expr_name, "sbox_sr") == 0)
		return test_aes_break_random_key_sbox_sr(argc, argv);
	...
	if(strcmp(expr_name, "ttable_msb") == 0)
		return test_aes_break_random_key_ttable(argc, argv);
	if(strcmp(expr_name, "ttable_sr") == 0)
		return test_aes_break_random_key_ttable_sr(argc, argv);
